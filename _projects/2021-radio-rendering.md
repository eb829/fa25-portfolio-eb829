...
### Final Assembly
The container includes a sliding drawer with 4 wheels mounted underneath, allowing the user to fully remove and empty the trap without disassembling the entire system. This is especially important for field use, since the trap may need to be emptied multiple times during harvest. To prevent the easily removable drawer from leaving the trap at undesired times an L bracket is attached to either side of the drawer on one side and the housing on the other. 
...
<a href="/assets/Bill-of-Materials.pdf" target="_blank">View Bill of Materials PDF</a> 
